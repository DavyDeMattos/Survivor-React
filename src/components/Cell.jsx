...
export function Cell({ type,people, onClick }) {
    ...
    return (
        <div className="relative flex justify-center items-center border-1 border-blue-200 hover:bg-blue-200" onClick={onClick}>
            ...
            { people > 0 && <div className="absolute inline-flex items-center justify-center w-6 h-6 text-xs font-bold text-black bg-white border-2 border-white rounded-full top-1 end-1 dark:border-gray-900">{people}</div>}
        </div>
    );
}